...
class RecordedData: 
    def __init__(self):
        ...
        self.PERCENT_OF_STD_TO_TRADE_AT = 0.7
...
class Trader:
    ...
    def run(self, state: TradingState) -> tuple[dict[Symbol, list[Order]], int, str]:
        ...
        self.CHOCOLATE_CACHE_SIZE = data.STARFRUIT_CACHE_SIZE
        self.ROSE_CACHE_SIZE = data.STARFRUIT_CACHE_SIZE
        ...
        for product in state.order_depths:
            ...
            if product == "AMETHYSTS":
                ...
                if difference > data.PERCENT_OF_STD_TO_TRADE_AT * data.DIFFERENCE_STD: # basket overvalued, sell
                    orders += self.calculate_orders(product, order_depth, -self.INF, worst_bid_price, gift_basket=True)
                
                elif difference < -data.PERCENT_OF_STD_TO_TRADE_AT * data.DIFFERENCE_STD: # basket undervalued, buy
                    orders += self.calculate_orders(product, order_depth, worst_ask_price, self.INF, gift_basket=True)

            # elif product == "STRAWBERRIES":
            #     # keep the length of strawberry cache as STRAWBERRY_CACHE_SIZE
            #     if len(data.STRAWBERRY_CACHE) == self.STRAWBERRY_CACHE_SIZE:
            #         data.STRAWBERRY_CACHE.pop(0)

            #     _, best_sell_price = self.get_volume_and_best_price(order_depth.sell_orders, buy_order=False)
            #     _, best_buy_price = self.get_volume_and_best_price(order_depth.buy_orders, buy_order=True)

            #     data.STRAWBERRY_CACHE.append((best_sell_price+best_buy_price)/2)
            ...
            #     if len(data.STRAWBERRY_CACHE) == self.STRAWBERRY_CACHE_SIZE:
            #         lower_bound = self.estimate_lobf_price(data.STRAWBERRY_CACHE)-2
            #         upper_bound = self.estimate_lobf_price(data.STRAWBERRY_CACHE)+2
