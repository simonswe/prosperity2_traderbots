    ...
    def run(self, state: TradingState) -> tuple[dict[Symbol, list[Order]], int, str]:
        ...
        for product in state.order_depths:
            ...
            if product == "AMETHYSTS":
                ...
                prediction = 4*mid_price['CHOCOLATE'] + 6*mid_price['STRAWBERRIES'] + mid_price['ROSES'] + data.DIFFERENCE_MEAN
                orders += self.calculate_orders(product, order_depth, int(round(prediction))-2, int(round(prediction))+2)
                ...
                #     # items undervalued, buy
                #     # for item in ['CHOCOLATE', 'STRAWBERRIES', 'ROSES']:
                #     #     result[item] = self.calculate_orders(item, state.order_depths[item], worst_ask_price[item], self.INF, mm_at_our_price=True)                
                ...
                #     orders += self.calculate_orders(product, order_depth, best_ask_price['GIFT_BASKET'], self.INF, mm_at_our_price=True)
                    ...
                    # items overvalued, sell
                    # for item in ['CHOCOLATE', 'STRAWBERRIES', 'ROSES']:
                    #     result[item] = self.calculate_orders(item, state.order_depths[item], -self.INF, worst_bid_price[item], mm_at_our_price=True)

                # if difference is around 0, we want a neutral position
                # if abs(difference) < 0.05 * data.DIFFERENCE_STD:
                #     logger.print('smal difference')
                #     if self.POSITION['GIFT_BASKET'] > 0: # if we are net long, sell to convert back to 0
                #         orders += self.calculate_orders(product, order_depth, -self.INF, worst_bid_price['GIFT_BASKET'], orchild=True, mm_at_our_price=True)
                #     elif self.POSITION['GIFT_BASKET'] < 0: # if we are net short, buy to convert back to 0
                #         orders += self.calculate_orders(product, order_depth, worst_ask_price['GIFT_BASKET'], self.INF, orchild=True, mm_at_our_price=True)
