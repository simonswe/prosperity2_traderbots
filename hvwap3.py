    ...
    def run(self, state: TradingState) -> tuple[dict[Symbol, list[Order]], int, str]:
        ...
        for product in state.order_depths:
            ...
            if product == "AMETHYSTS":
                ...
                sell_vwap = self.calculate_vwap(order_depth.sell_orders)
                buy_vwap = self.calculate_vwap(order_depth.buy_orders)
                vwap = (sell_vwap + buy_vwap) / 2 if buy_vwap and sell_vwap else max(sell_vwap, buy_vwap)
                
                if len(order_depth.sell_orders) != 0:
                    best_ask, best_ask_amount = list(order_depth.sell_orders.items())[0]
                    if best_ask <= vwap:
                        print("BUY", product, str(-best_ask_amount) + "x", best_ask)
                        orders.append(Order(product, best_ask, -best_ask_amount))

                if len(order_depth.buy_orders) != 0:
                    best_bid, best_bid_amount = list(order_depth.buy_orders.items())[0]
                    if best_bid >= vwap:
                        print("SELL", product, str(-best_bid_amount) + "x", best_bid)
