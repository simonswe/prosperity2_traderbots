        ...
        for product in state.order_depths:
            ...
            if product == "AMETHYSTS":
                ...
                if data.PREV_COCONUT_PRICE != -1:
                    if mid_price > data.PREV_COCONUT_PRICE:
                        # last time stamp rose, expect next time stamp to drop, short
                        orders += self.calculate_orders(product, order_depth, -self.INF, best_buy_price)
                    elif mid_price < data.PREV_COCONUT_PRICE:
                        # last time stamp dropped, expect next time stamp to rise, long
                        orders += self.calculate_orders(product, order_depth, best_sell_price, self.INF)

                # update previous coconut price
                data.PREV_COCONUT_PRICE = mid_price
